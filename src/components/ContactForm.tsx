...
const schema: ZodType<FormData> = z.object({
  ...
  phoneNumber: z.string().min(10, { message: 'Phone number should be at least 10 characters' }),
  ...
  return (
    <section id="join" className="py-20 bg-white relative">
      <div className="flex flex-col items-center px-4">
        <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-6 md:mb-7 lg:mb-14 text-center text-accent-color mb-8">Contact <span className="font-extrabold">Us</span></h2>
        <div className="flex flex-col lg:flex-row w-full lg:justify-center">
          <div className="lg:w-1/2 items-center self-center lg:self-start flex flex-col justify-center">
            <p className="text-center font-bold text-text-color font-raleway text-xs md:text-lg lg:text-xl lg:text-left max-w-xs md:max-w-sm lg:max-w-md mb-8 lg:mt-4 h-full flex items-center">We&lsquo;re here to make &apos;ship&apos; happen seamlessly. Led by the remarkable Filbert Wingrove, our team turns shipping worries into a breeze.</p>
          </div>
          <div className="lg:w-1/2">
            <form className="lg:max-w-lg w-full" onSubmit={handleSubmit(onSubmit)}>
              {formInputs.map((input, index) => (
                <div key={index} className="mb-4">
                  <label className="block font-raleway text-xs md:text-xl text-left text-text-color">{input.label}</label>
                  <input
                    type={input.type}
                    {...register(input.name)}
                  ...
                  {errors[input.name as keyof FormData] && (
                    <span className="text-red-500">{errors[input.name as keyof FormData]?.message}</span>
                  )}
                </div>
              ))}
              <button type="submit" className="w-full bg-accent-color text-white font-bold py-2 px-4 rounded-lg">Submit</button>
            </form>
          </div>
        </div>
      </div>
